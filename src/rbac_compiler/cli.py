...
from .errors import CompilerInternalError, RegistryLoadError
from .loader import discover_org_files, load_agent_registry, load_org_file, load_vocabulary
...
def cli(
    ...
    Reads classification_vocabulary.yml, orgs/*.yml, and agent_registry.yml
    ...
    if output is None:
        output = registry_dir / ".compiled" / "compiled_plan.yml"
    ...
    try:
        ...
        logger.info("Loaded vocabulary: %s", vocab_path)

        agent_registry, agents_hash = load_agent_registry(agents_path)
        logger.info("Loaded agent registry: %s (%d agents)", agents_path, len(agent_registry.agents))

        org_paths = discover_org_files(registry_dir)
        if not org_paths:
            click.echo(f"WARN: No org files found in {registry_dir / 'orgs'}", err=True)
    ...
    result: ValidationResult = validate_all(
        vocab, vocab_path, org_files, agent_registry, agents_path
    )
